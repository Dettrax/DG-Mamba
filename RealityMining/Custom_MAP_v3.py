...
class LinkPredictor(nn.Module):
    def __init__(self, feature_dim, hidden_dim=64):
        ...
        self.graph_encoder = nn.Sequential(
            nn.Linear(96, hidden_dim),  # Assuming max nodes is 96
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim)
        )
    ...
    def forward(self, history_graphs, history_features):
        batch_size = history_graphs.size(0)
        seq_len = history_graphs.size(1)
        num_nodes = history_graphs.size(2)

        # Process node features
        node_embeddings = self.feature_encoder(history_features.view(-1, history_features.size(-1)))
        node_embeddings = node_embeddings.view(batch_size, seq_len, num_nodes, -1)
        ...
        # Combine embeddings
        combined_embeddings = node_embeddings + graph_embeddings

        # Apply temporal attention
        # Reshape for attention: [seq_len, batch * nodes, hidden]
        combined_embeddings = combined_embeddings.permute(1, 0, 2, 3)
        combined_embeddings = combined_embeddings.reshape(seq_len, batch_size * num_nodes, self.hidden_dim)

        attended = self.temporal_attention(combined_embeddings)

        # Reshape back: [batch, nodes, hidden]
        attended = attended[-1].view(batch_size, num_nodes, self.hidden_dim)

        # Predict edges
        # Create all possible node pairs
        node_i = attended.unsqueeze(2).expand(-1, -1, num_nodes, -1)
        node_j = attended.unsqueeze(1).expand(-1, num_nodes, -1, -1)

        # Concatenate node pairs
        edge_features = torch.cat([node_i, node_j], dim=-1)

        # Predict edge probabilities
        edge_probs = self.edge_predictor(edge_features.view(-1, self.hidden_dim * 2))
        edge_probs = edge_probs.view(batch_size, num_nodes, num_nodes)

        return edge_probs


def train_and_evaluate(temporal_data, train_ratio=0.8, epochs=50, batch_size=32):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # Split dataset
    train_size = 63  # First 63 timestamps as mentioned
    ...
    # Initialize model
    model = LinkPredictor(feature_dim=7).to(device)  # 7 features as per your dataset
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            target = batch['target'].squeeze(1).to(device)  # Remove predict_window dimension

            optimizer.zero_grad()
            pred = model(history_graphs, history_features)
...
if __name__ == "__main__":
    ...
    # Train and evaluate
    model, best_map = train_and_evaluate(temporal_data)

...
class GraphModel(torch.nn.Module):
    def __init__(self, channels: int, pe_dim: int, num_layers: int, model_type: str, shuffle_ind: int, d_state: int,
        ...
        for _ in range(num_layers):
            nn = Sequential(
                Linear(self.process_dim, self.channels),
                ReLU(),
                Linear(self.channels, self.process_dim),
            )
            if self.model_type == 'gine':
                conv = GINEConv(nn)

            if self.model_type == 'mamba':
                conv = MambaConv(self.process_dim ,GINEConv(nn), heads=4, attn_dropout=self.dropout,
                               att_type='mamba',
        ...
        self.mu = Linear(self.process_dim, 64)
        self.sigma = Linear(self.process_dim, 64)
        self.elu = ELU()
        ...
        self.x_linear = Linear(self.process_dim, self.process_dim).to(device)
        self.node_emb_norm_after = BatchNorm1d(self.process_dim).to(device)
...
def optimise_mamba(lookback,window_size,stride,channel,pe_dim,num_layers,d_conv,d_state,dropout,lr,weight_decay,walk_length):
    ...
    for e in tqdm(range(50)):
        ...
        with torch.no_grad():
            model.eval()
            for i in range(lookback +int(diff * 0.7), lookback +int(diff * 0.8)):
                x, pe, edge_index, edge_attr, batch, triplet, scale = dataset[i]
                optimizer.zero_grad()
...
lookback = 20
walk = 16
#{'lookback': 5, 'stride': 1, 'channel': 16, 'pe_dim': 2, 'num_layer': 2, 'd_conv': 2, 'd_state': 2, 'dropout': 0.49678480649939993, 'lr': 0.0067408592018342135, 'weight_decay': 0.0038868933420821566, 'walk': 16}
model , val_losses , loss_step = optimise_mamba(lookback=lookback,window_size=96,stride=1,channel=32,pe_dim=2,num_layers=1,d_conv=6,d_state=32,dropout=0.42,lr=0.0035,weight_decay=0.00094,walk_length=walk)

...
def optimise_mamba(lookback, window_size, channel, pe_dim, num_layers, d_conv, d_state, dropout, lr,
    ...
    for i in range(3):
        ...
        MAPS.append(MAP)
    #return median of MAPS
    return sorted(MAPS)[0]

 # MAP = optimise_mamba(2,96,8,4,1,6,16,0.44,8.63e-05,0.016,16)
...
def train_model(config):
    map_value = optimise_mamba(
        lookback=config['lookback'],
        window_size=96,  # Assuming lin_dim is intended to be window_size
        channel=config['channel'],  # Ensure this is defined in your search space or config
        pe_dim=4,
        ...
        walk_length=16  # Ensure walk_length is also defined in your config
    )
    return map_value
...
search_space = {
    ...
    "channel": tune.randint(8, 32),
    "num_layers": tune.randint(1, 5),
    "d_conv": tune.randint(2, 7),
    "d_state": tune.randint(2, 20),

...
import torch.nn as nn
# Add this code before the calculate_flops call
class ModelWrapper(nn.Module):
    ...
    def forward(self, x):
        # Generate dummy edge_list for each timestep

        edge_list = []
        for j in range(x.shape[1]):  # For each timestep
            # Create a sparse edge_index (just minimal connections for calculation)
            # This is a simplification but works for FLOP counting
            edge_index = torch.zeros((2, x.shape[0]), dtype=torch.long)
            edge_index[0] = torch.arange(x.shape[0])
            edge_index[1] = (torch.arange(x.shape[0]) + 1) % x.shape[0]
            edge_list.append(edge_index.to(device))

        return self.model(x.to(device), edge_list)


# Use the wrapper for FLOP calculation
wrapped_model = ModelWrapper(t)
flops, macs, params = calculate_flops(
    model=wrapped_model.to(device),
    input_shape=(96, lookback + 1, 96),

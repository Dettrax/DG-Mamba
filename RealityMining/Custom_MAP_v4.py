...
class LinkPredictor(nn.Module):
    def __init__(self, num_nodes=96, hidden_dim=64):
        ...
        # Temporal attention
        self.temporal_attention =   Mamba(d_model=hidden_dim,d_state=16,d_conv=3)
    ...
    def forward(self, history_graphs):
        ...
        attended = self.temporal_attention(embeddings) # batch* nodes , seq_len, hidden
...
def train_and_evaluate(temporal_data, epochs=50, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        train_targets = np.concatenate([t.reshape(-1) for t in train_targets])
        train_map_score = average_precision_score(train_targets, train_preds)
        ...
        if (epoch + 1) % 5 == 0:
            ...
            print(f'Training MAP Score: {train_map_score:.4f}')
            print(f'Test MAP Score: {map_score:.4f}')

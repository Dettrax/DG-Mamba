...
class GraphModel(torch.nn.Module):
    def __init__(self, channels: int, pe_dim: int, num_layers: int, model_type: str, shuffle_ind: int, d_state: int,
        ...
        self.edge_emb = Embedding(2, self.process_dim)
        ...
        self.mu = Linear(self.channels, self.channels)
        self.sigma = Linear(self.channels, self.channels)
        self.elu = ELU()
        self.convs_norm = ModuleList([BatchNorm1d(self.process_dim) for _ in range(num_layers)])
        self.conv_mamba = BatchNorm1d(self.channels).to(device)
        ...
        self.gcn = GCNConv(self.channels, self.channels)
        config = MambaConfig(d_model=self.channels,d_state = d_state,d_conv=d_conv, n_layers=num_layers,expand_factor=1)
    ...
    def forward(self, x, pe, edge_index, edge_attr, batch):
        ...
        x_after_drop = self.gcn(x_after_drop, edge_index)

        h, mask = to_dense_batch(x_after_drop, batch)
        h = self.self_attn(h)
        x_after_drop = h[mask]
        x_after_norm = self.conv_mamba(x_after_drop)
        x_after_drop = self.dropout_ly(x_after_norm)
    ...
    for e in tqdm(range(50)):
        ...
        for i in range(lookback, 63):
                ...
                mu, sigma = model(x, pe, edge_index, edge_attr,
                    batch)
                loss = build_loss(triplet, scale, mu, sigma, channel, scale=False)
        ...
        with torch.no_grad():
            ...
            for i in range(63, 72):
                ...
                mu, sigma = model(x, pe, edge_index, edge_attr,
                                  batch)
                curr_val_loss = build_loss(triplet, scale, mu, sigma, channel, scale=False).item()
...
lookback = 2
walk = 8
#s model , val_losses , loss_step = optimise_mamba(lookback=lookback,window_size=96,stride=1,channel=8,pe_dim=8,num_layers=2,d_conv=2,d_state=32,dropout=0.38,lr=0.0054,weight_decay=0.0015,walk_length=walk)

model , val_losses , loss_step = optimise_mamba(lookback=lookback,window_size=96,stride=1,channel=16,pe_dim=4,num_layers=2,d_conv=2,d_state=4,dropout=0.35,lr=0.0001,weight_decay=0.001,walk_length=walk)

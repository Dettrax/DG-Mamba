...
def get_inf(data, mu_64, sigma_64, lookback):
    return_dict = {}
    count = 0
    mu_64 = [torch.tensor(mu, device=device) for mu in mu_64]
    ...
    for ctr in range(lookback, 35):
        ...
        if count > 0:
            ...
            if ctr < 35:
                ...
                val_ground_truth = torch.tensor(A.toarray()).to(device)

                val_ground_truth = val_ground_truth[val_edges[:, 0], val_edges[:, 1]]

                a, b = unison_shuffled_copies(val_edges, val_ground_truth, count)

                if ctr >= 0:
                    a_embed = mu_64[ctr - lookback][a]
                    a_embed_stacked = a_embed.view(-1, a_embed.size(2) * 2)  # Reshape for the classifier

                    inp_clf = a_embed_stacked.to(device)
                    b = b.to(device)

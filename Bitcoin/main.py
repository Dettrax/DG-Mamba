...
for i in range(lookback, 137):    #lookback + 1 because ignore timestamp 2
    ...
    print(f"Timestamp: {i}, Adj matrix shape: {adj_matr.shape} , Dataset shape: {B.shape}")
    dataset[i] = B
...
def val_loss(t):
    l = []
    for j in range(95,109):
        _,muval,sigmaval = t(torch.tensor(dataset[j],dtype = torch.float32).to(device))
...
t.load_state_dict(torch.load('model30.pth'))
